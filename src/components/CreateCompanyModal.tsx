...
import {Article} from "../interfaces/article.interface.ts";
...
const CreateCompanyModal: React.FC<CreateCompanyModalProps> = ({ open, onClose }) => {
  ...
  const { mutateAsync, isPending } = useCreateCompanyQuery();
  const { data: articles } = useGetAllArticlesQuery({ name: '' });
  ...
  const handleSubmit = async () => {
    try {
      await mutateAsync({ ...newCompany, articleIds: selectedArticles });
      setNewCompany({
        name: '',
      });
      setSelectedArticles([]);
      ...
      <Dialog open={open} onClose={onClose}>
        ...
        <DialogContent>
          ...
          <Autocomplete
              ...
              getOptionLabel={(option: Article) => option.name}
              onChange={(_e, value) => handleArticleChange(value.map((article: Article) => article._id))}
              renderInput={(params) => (
                  <TextField
                      {...params}
                      margin="dense"
                      label="Parfemi"
                      placeholder="Izaberi parfeme"
                      fullWidth
                  />
              )}
          />
